...
// Mirrors web/public/mock-tour-route-mexico-7day.csv — the rehearsal/show rows
...
2025-09-22,leg_mx,travel,LAX → CDMX,MX,,,,,,,,,,Tour kickoff. Group flight AM 19.
2025-09-23,leg_mx,rehearsal,Mexico City,MX,Foro Indie Rocks (rented),"Calle Zacatecas 39, Roma Norte, 06700 CDMX",,,,,,Self,,Production rehearsal.
2025-09-24,leg_mx,rehearsal,Mexico City,MX,Foro Indie Rocks (rented),"Calle Zacatecas 39, Roma Norte, 06700 CDMX",,,,,,Self,,Production rehearsal.
2025-09-25,leg_mx,show,Mexico City,MX,Auditorio Nacional,"Paseo de la Reforma 50, Bosque de Chapultepec I Secc, 11580 CDMX",9683,65000,20:00,21:30,23:30,OCESA,Pablo Espinosa,Tier A.
2025-09-26,leg_mx,off,Mexico City,MX,,,,,,,,,,Day off.
2025-09-27,leg_mx,travel,CDMX → MTY,MX,,,,,,,,,,VivaAerobus VB 1014.
2025-09-28,leg_mx,show,Monterrey,MX,Auditorio Banamex,"Av. Fundadores S/N, La Estanzuela, 64988 Monterrey",8500,45000,20:00,21:30,23:30,OCESA,Pablo Espinosa,`;
...
describe('parseRouteCsv', () => {
  ...
  it('sets tour start/end from the date span', () => {
    expect(route.startDate).toBe('2025-09-22');
    expect(route.endDate).toBe('2025-09-28');
  });
  ...
  it('builds one leg covering the routed dates', () => {
    ...
    expect(route.legs[0].startDate).toBe('2025-09-22');
    expect(route.legs[0].endDate).toBe('2025-09-28');
  });

  it('handles quoted address fields with embedded commas', () => {
    const rehearsal = route.days.find((d) => d.date === '2025-09-23');
  ...
  it('attaches a slugged venueId to show days only', () => {
    expect(route.days.find((d) => d.date === '2025-09-25')?.venueId).toBe('v_auditorio_nacional');
    expect(route.days.find((d) => d.date === '2025-09-28')?.venueId).toBe('v_auditorio_banamex');
    expect(route.days.find((d) => d.date === '2025-09-23')?.venueId).toBeUndefined();
  });
  ...
  it('seeds a schedule skeleton for every day type', () => {
    ...
    expect(countFor('2025-09-22')).toBe(3); // travel
    expect(countFor('2025-09-23')).toBe(5); // rehearsal
    expect(countFor('2025-09-25')).toBe(9); // show
    expect(countFor('2025-09-26')).toBe(2); // off
    expect(route.scheduleItems).toHaveLength(36);
  });

  it('builds a full show-day skeleton', () => {
    const cdmx = route.scheduleItems.filter((s) => s.dayId === 'day_2025-09-25');
  ...
  it('puts soundcheck exactly 6 hours after load-in (rider §10 minimum)', () => {
    const cdmx = route.scheduleItems.filter((s) => s.dayId === 'day_2025-09-25');
