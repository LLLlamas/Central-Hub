...
- **Persistence:** two localStorage keys, both per-browser. (1) `tour-hub:scratch-tour`
  ...
  `pendingVisibilityEdits`, `visibilityEditHistory`, `sectionEdits`, `pendingEdits`,
...
- **Conflict detector scoped to genuine intra-doc mistakes** — `detectConflicts()` no longer flags §11 air tickets vs §12 occupants (intentional — local hires don't fly). The §11 ticket regex is now `extractFlightTickets` in `lib/pdfCore.mjs` and requires "boletos de avión" specifically (bare "boletos"/"pasajes" collided with "20 pasajeros"). The §12 header-vs-rows check stays. Cross-document conflicts (new rider vs old, rider vs travel grid, …) parked in `redesign-plan.md`.
