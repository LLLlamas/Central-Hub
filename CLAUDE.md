...
- **PDF references open in-app, never a new tab.** Call `usePdfViewer().openPdf({ url, page?, title? })` (`components/PdfViewer.tsx`). The active rider PDF is whatever the user uploaded — reach it via `useActiveRiderPdfUrl()` in `RiderIngest.tsx` (returns `string | undefined`) or directly via `useApp().tour.riderImports[0]?.pdfObjectUrl`. When the URL is undefined (no rider imported yet), HIDE the open-PDF affordance — don't fake a disabled link, don't fall back to a canonical fixture. `RIDER_PDF_PATH` from `lib/riderSections.ts` is now fixture-seed only: the canonical Spanish PDF path that `data/riderFixture.ts` and `hydrateRiderPlotImages` use when the fixture-fallback path runs. Don't add `<a href="….pdf" target="_blank">`.
...
- **In-house PDF parser — TOC-driven** (`lib/pdfParser.ts` + `lib/pdfCore.mjs`) — no LLM, no backend. pdfjs-dist text extraction + positional reconstruction. Handles Spanish and English riders. Three exports: `parseRiderPdf(file) → RiderImport`, `parseFlightPdf(file, personnel) → FlightImport | null`, `parseHotelPdf(file, personnel, days) → { hotels, tasks }`. Wired into `RiderIngest`, `FlightIngest`, and `HotelImportSection` upload handlers — each tries the real parser first, falls back to the fixture if parsing fails or returns empty. **Shared module:** `lib/pdfCore.mjs` holds all pure algorithmic helpers (constants, `groupRows`, `buildColMap`, `assignCols`, `multiPageItems`, `colAliasLookup`, `parseTocEntries`, `findHeadingPages`, `isPlotPage`, `classifyTocTitle`, etc.) imported by both `pdfParser.ts` (Vite/browser) and `scripts/parse-rider.mjs` (Node CLI). TypeScript resolves `.mjs` imports via `pdfCore.d.mts` (not `.d.ts` — `moduleResolution: "bundler"` maps `.mjs` → `.d.mts`). **TOC drives sectioning.** Rider parsing now reads the rider's own table of contents and produces exactly one `RiderSection` per TOC entry (tagged with `tocIndex` + verbatim `title` + `endPage`). `parseTocEntries` scores each early page by how many `^N. Title$` rows it contains plus the count of monotonic numeric sequences, returning the best in §N order with a non-enumerable `tocPage` hint. `findHeadingPages` then locates each entry's body heading via a two-pass match: a strict `^N- ALLCAPS$` pass first (the canonical "1- INTRO" form), then a loose `^N. Title-Case$` fallback that excludes the TOC page so the TOC itself isn't picked up as §1's start. End pages computed as `Math.max(start, nextStart-1)`. Each entry's title is classified via `classifyTocTitle` and routed into the right typed extractor (`extractInputList`, `extractRooming`, `extractBackline`, `extractCatering`) or a free-text section. **Fallback:** when TOC parsing fails or yields <5 resolvable headings, the parser uses `buildLegacySections` which runs the old heading-regex pass (`SECTION_HINTS`), so non-canonical riders don't regress. **Plot pages:** `isPlotPage(page)` flags pages with <8 text items — these get attached to the owning section as `RiderSection.plots[]` references (`{ page, caption, kind }`) instead of fed through text extraction. The Rider Ingest sidebar has a dedicated "Plots" entry that grids every plot image with a "Open page N" link. **Per-page free text + chrome strip + cross-section clip.** Free-text sections expose `RiderSection.pageTexts: { page, text }[]` (one entry per source page in the section's range), with `freeText` set to the join — `extractPageText(page, { clipAboveY?, keepFromY?, headerBandPt?, footerBandPt? })` strips repeating page chrome (top header band, bottom footer / page numbers, repeated rider title via `CHROME_PATTERNS`) and clips bounds when adjacent sections share a page: `findHeadingY(page, num)` finds the current section's own heading y → `keepFromY` drops the previous section's leftover content above it; `findNextHeadingY(page, afterNum)` finds the next numbered heading on the section's last page → `clipAboveY` drops the next section's content below it. So §2 NOTAS and §3 PERMISOS — both starting on page 3 of the canonical rider — render as just their own heading + subheading + paragraph, with no bleed. The `SectionView`'s `FreeTextReview` renders multi-page sections as one labeled `PageTextBlock` per page (with an "Open page N" link to the in-app PDF viewer); single-page sections render as a single block without page chrome. The bottom textarea still edits the joined `freeText` as a whole — per-page inline editing isn't built. **Known quirks:** pdfjs-dist fragments Spanish accented chars (`producción` → `producci ó n`) and ligatures (`Especi fi caciones`) — language detection uses unaccented Spanish function words; column alias lookup uses prefix matching for fragmented headers; TOC titles are preserved verbatim with their fragmentation because the UI shows them in the source language. Multi-page sections use the y-offset trick (`(nPages-1-pi)*10000`) so items from different pages don't collide in `groupRows`. The `input_list` regex includes `\bCH\s+SOURCE\b` so continuation pages (same table header, no section-title text) are included in the span. The canonical rider extracts 14 TOC sections + 44 channels + 8 monitor mixes + 5 FOH outputs + 5 plot pages (1 stage plot + 4 lightplot sheets). **CLI:** `node scripts/parse-rider.mjs [path/to/rider.pdf]` writes `src/data/riderExtracted.json`; useful for testing extraction quality. **Uploaded PDF persistence (`web/src/lib/riderPdfStore.ts`).** localStorage strips `pdfObjectUrl` on save (Blob URLs are session-scoped anyway); the raw PDF bytes are persisted to IndexedDB (one object store, keyed by `RiderImport.id`) so a hard refresh can rehydrate the Blob URL without a re-upload. Native IndexedDB API, no deps. `saveRiderPdf(id, bytes)` runs after a successful parse (and on the fixture-fallback path, which fetches `RIDER_PDF_PATH` so the canonical Spanish bytes are stashed too). `AppStateProvider` has a boot effect keyed on `riderImports[0].id` that calls `loadRiderPdf`, mints a `Blob` URL, and patches `pdfObjectUrl` back onto the import. `cancelRiderImport(id)` calls `deleteRiderPdf(id)`; `resetScratchTour()` calls `clearAllRiderPdfs()`.
...
- **Rider section review — TOC-driven two-pane surface.** `/ingest/riders` is restructured around the rider's table of contents: the left rail lists exactly the §N entries the parser found (verbatim TOC titles, source language) + a dedicated "Plots" entry below them. Selecting a section opens a two-pane detail view — embedded PDF on the left (`PdfViewerInline`, scoped to that section's pages via `#page=N`), extracted/editable content on the right (same `EditableText` / `EditableSelect` primitives the day-sheet edit mode uses). **Active rider PDF resolution.** The embedded viewer + every "open the rider PDF" affordance (header filename chip, `SectionPageLinks`, plot tiles, `RiderRef` §N links, `ExplainTag` rider-page jumps, provenance "view PDF" buttons in `SourceTag` / `MockTag`) reads from `tour.riderImports[0]?.pdfObjectUrl`. When no rider has been imported (or the boot rehydrate hasn't finished), those affordances are **hidden, not faked** — `useActiveRiderPdfUrl()` returns `undefined`; consumers omit iframes, drop click handlers, and render `§N` references as plain text. Provenance artifact URLs use a tagged-resolver shape (`url: string | { kind: 'active_rider_pdf' }`) so the same data can point to a public fixture URL or the active rider's Blob URL; `resolveProvenanceUrl(url, tour)` in `data/sources.ts` does the lookup at render time. The reviewer's job is to confirm the parser pulled the right text out of the PDF, edit anything wrong, then Approve. The approval / pending-edit / per-section history flow is unchanged (still keyed `${type}-${index}` via `updateSectionEdit` / `proposeSectionEdit` / `approveSection`). The Intro section is the lead simple case — the embedded PDF shows page 2 + the extracted blurb sits next to it for verbatim comparison. The Input List, Monitor Mix, FOH Outputs tables and free-text sections are inline-editable; Backline, Catering, Lodging are review-only for now — see the gap list. **Plot-type sections (`stage_plot`, `lighting_plot`, or any section carrying `plots[]`) skip the text-extraction surface entirely**: `SectionReviewSplit` routes them to `PlotSectionReview`, a single-pane image stack with Approve/Reopen — no PDF iframe, no editable textarea. Approval semantics shift from "the extracted text matches" to "these are the right images." The old pipeline strip, filter chips, "Sections / Visuals" toggle, and the trailing "Conflicts & Notes" rail entry were all removed in earlier restructures — conflict data still flows through `ConflictFeed` on the Tour Overview via AppState, just not as a per-section row at this altitude.
- **Plots tab + top-level `/plots`** — `RiderIngest`'s left rail has a "Plots" entry below the 14 sections that renders a **thumbnail grid** (`grid-cols-2 lg:grid-cols-3 xl:grid-cols-4`, ~4:3 aspect tiles, `object-contain` so plots aren't cropped) of every plot image the parser extracted (stage plot, lightplot sheets). Each tile: clicking the image opens the source page in the in-app PDF viewer; a tiny "§N review" link below jumps to that section's single-pane image view; hover shows a "Click to enlarge" hint via group-hover ring. The same `PlotCard` component is reused by **`/plots`** — a top-level surface (sidebar entry below "People", `/more` overflow entry, Cmd+K page index) that grids every rider plot independent of the ingest review flow, with a "View rider" link back to `/ingest/riders`. The rider-ingest Plots panel cross-links to `/plots` via a small "Open Plots ›" link in the header. Empty state on `/plots` covers the "no rider imported" + "rider has no plot pages" cases. Each card shows the rendered PNG inline (no iframe). Plot images are surfaced as `RiderSection.plots[]` (`{ page, caption, kind, dataUrl?, width?, height? }`) and detected via `isPlotPage` (any page with <8 text items inside a section's range). The `dataUrl` is a PNG rendered from the PDF page via pdfjs canvas (`renderPagesToPng` / `renderPlotImagesFromUrl` in `lib/pdfParser.ts`, scale 2x). Two write paths: `parseRiderPdf` renders inline after section detection; the fixture path (`buildScratchRiderImport` → `hydrateRiderPlotImages`) fetches `RIDER_PDF_PATH` and renders on import. **Not persisted to localStorage** — base64 PNGs would blow the quota; `scratchStorage.ts saveScratchTour` strips `dataUrl`/`width`/`height` before serializing, and `AppStateProvider` runs a boot effect (keyed on `riderImports[0].id`) that re-derives them from the source PDF whenever a restored tour is missing them.
